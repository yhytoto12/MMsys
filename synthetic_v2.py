...
def read_image_v2(res1, res2):
    return np.random.randint(0, 256, (res1, res2, 3), dtype=np.uint8)

def read_audio():
    return np.random.randn(3, 16000).astype(np.float32)

class RealTimeDataMulti(IterableDataset):
    def __init__(self, transforms, read_funcs):
...
if __name__ == '__main__':
    ...
    ress = [()]
    read_funcs = [read_image, read_audio]
    ...
    with profile(
        ...
        on_trace_ready = tensorboard_trace_handler('./final_log/synthetic-3', f'{n_modes}-{run_type}_{os.getpid()}')
    ) as prof:
        if 'naive0' in run_type:
            dataset = RealTimeDataMulti(transforms, read_funcs)
            ...
            datasets = [RealTimeData(transforms[i], read_funcs[i], i) for i in range(n_modes)]
            dataloaders = [
