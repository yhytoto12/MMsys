...
class AVSR(torch.nn.Module):
    def __init__(self, modality, model_path, model_conf, rnnlm=None, rnnlm_conf=None,
        ...
        print(f"model init device {self.device}")
    ...
    def infer(self, data):
        with torch.no_grad():
            if isinstance(data, tuple):
                print(f"enc device {self.device}")
                enc_feats = self.model.encode(data[0].to(self.device), data[1].to(self.device)) #.to(self.device)
            else:
                enc_feats = self.model.encode(data.to(self.device))
            with record_function("BEAM SEARCH"):
